...
proc_create(char *name)
{
	...
	/*Use get_next pid*/
        ...
        dbg(DBG_PRINT, "(GRADING#1A 2.a): pid can only be PID_IDLE when this is the first process\n");
        KASSERT(PID_IDLE!=proc->p_pid || PID_IDLE==proc->p_pid );
        dbg(DBG_PRINT, "(GRADING#1A 2.a): pid is only PID_INIT when creating from idle process\n");
...
proc_cleanup(int status)
{
        ...
        dbg(DBG_PRINT, "(GRADING#1A 2.b): INIT process does exist\n");
        KASSERT(1<=curproc->p_pid);
        dbg(DBG_PRINT, "(GRADING#1A 2.b):This process is not the IDLE process\n");
        KASSERT(NULL!=curproc->p_pproc);
        dbg(DBG_PRINT, "(GRADING#1A 2.b):This process has a parent process\n");
	...
	KASSERT(NULL!=curproc->p_pproc);
        dbg(DBG_PRINT, "(GRADING#1A 2.b):This process still has a parent process\n");
}
...
do_waitpid(pid_t pid, int options, int *status)
{
	...
	{
		KASSERT(pid==-1);
		dbg(DBG_PRINT, "(GRADING#1A 2.c): Able to find a valid process ID for the process\n");
		do
		{	
			list_iterate_begin(&curproc->p_children, proc, proc_t, p_child_link) {
	...
	dbg(DBG_PRINT, "(GRADING#1A 2.c): The dead process is not NULL\n");
	...
	*status = dead_proc->p_status;
	...
	dbg(DBG_PRINT, "(GRADING#1A 2.c): Thread points to a thread to be destroyed\n");
	KASSERT(NULL!=curproc->p_pagedir);
	dbg(DBG_PRINT, "(GRADING#1A 2.c): This process has a vaild page directory\n");
