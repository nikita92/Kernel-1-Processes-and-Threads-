...
*/
	...
	dbg(DBG_PRINT, "(GRADING#1A 1.a): Idle process has been created successfully\n");	
        KASSERT(PID_IDLE == curproc->p_pid);
        dbg(DBG_PRINT, "(GRADING#1A 1.a): The process that has been created is indeed the IDLE process\n");
        	
	kthread_t * thread = kthread_create(proc, idleproc_run, 0, NULL); 
        curthr = thread;
        KASSERT(NULL != curthr);
        dbg(DBG_PRINT, "(GRADING#1A 1.a): The thread for the idle process has been created successfully\n");
...
test_proc_kill_all(kshell_t *ksh, int argc, char **argv){
	int rv;
	KASSERT(ksh != NULL);
        dbg(DBG_PRINT, "(GRADING#1E 1): test_proc_kill_all() is invoked, argc = %d, argv = 0x%08x\n",
                    argc, (unsigned int)argv);	
...
initproc_create(void)
{
	proc_t * proc = proc_create("init");
        KASSERT(NULL!=proc);
	dbg(DBG_PRINT, "(GRADING#1A 1.b): Init process has been created successfully\n");
	KASSERT(PID_INIT == proc->p_pid);
        dbg(DBG_PRINT, "(GRADING#1A 1.b): The process that has been created is indeed the INIT process\n");
	
	kthread_t * thread = kthread_create(proc, initproc_run, 0, NULL); 
	KASSERT(NULL != thread);
        dbg(DBG_PRINT, "(GRADING#1A 1.b): The thread for the idle process has been created successfully\n");
...
my_sunghan_test(kshell_t *ksh, int argc, char **argv){
	int rv;
	KASSERT(ksh != NULL);
        dbg(DBG_PRINT, "(GRADING#1D 1): sungham_process() is invoked, argc = %d, argv = 0x%08x\n",
                    argc, (unsigned int)argv);
...
my_sunghan_deadlock_test(kshell_t *ksh, int argc, char **argv){
	int rv;
	KASSERT(ksh != NULL);
        dbg(DBG_PRINT, "(GRADING#1D 2): sungham_process_deadlock() is invoked, argc = %d, argv = 0x%08x\n",
                    argc, (unsigned int)argv);	
...
my_faber_thread_test(kshell_t *ksh, int argc, char **argv){
	int rv;
	KASSERT(ksh != NULL);
        dbg(DBG_PRINT, "(GRADING#1C): faber_test() is invoked, argc = %d, argv = 0x%08x\n",
                    argc, (unsigned int)argv);
